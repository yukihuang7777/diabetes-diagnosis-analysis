...
```{R}
#| warning: false
# Check VIF and use backward method to choose model (base on AIC)
...
```{R}
# model selected by backward mehod
backward_selected_model <- glm(formula = Outcome ~ Age + BMI + HbA1c + FamilyHistory, data = diabetes_selected_eda, family = binomial)

reg_interationFH <- glm(formula = Outcome ~ Age + BMI + HbA1c + FamilyHistory + Age*FamilyHistory, data = diabetes_selected_eda, family = binomial)
reg_log <- glm(formula = Outcome ~ log(Age) + BMI + HbA1c + FamilyHistory + log(Age)*FamilyHistory, data = diabetes_selected_eda, family = binomial)
reg_sqrt <- glm(formula = Outcome ~ sqrt(Age) + BMI + HbA1c + FamilyHistory + sqrt(Age)*FamilyHistory, data = diabetes_selected_eda, family = binomial)
reg_scale <- glm(formula = Outcome ~ scale(Age) + BMI + HbA1c + FamilyHistory + scale(Age)*FamilyHistory, data = diabetes_selected_eda, family = binomial)
reg_onlyInteraction <- glm(formula = Outcome ~ scale(Age) + BMI + HbA1c + scale(Age)*FamilyHistory - FamilyHistory, data = diabetes_selected_eda, family = binomial)

summary(reg_interationFH)
summary(reg_log)
summary(reg_sqrt)
summary(reg_scale)
summary(reg_onlyInteraction)
```